...
const Sidebar = ({ isOpen, toggle}) => {
  return (
    <>
      <div
        className=" fixed w-full h-full overflow-hidden bg-white grid pt-[80px] left-0 z-40"
        style={{
          opacity: `${isOpen ? "0" : "1"}`,
          top: ` ${isOpen ? "-100%" : "0"}`,
        }}
      >
        <button value="close" className="absolute right-0 pr-3 pt-7 text-2xl font-bold text-gray-800" onClick={toggle}>
        {/* Close icon */}
        <MdOutlineClose />
        </button>
